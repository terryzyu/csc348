...
public class SymbolTable {
    ...
    public boolean enterScopeBool(String i, ASTNode n) {
    	SymbolTable st = this.getChild(i);
        if ( st == null ) {
            st = new SymbolTable(this, n);
            this.addChild(i, st);
    ...
    {
        TreeMap<String, Symbol> csorted = new TreeMap<>(); 
        TreeMap<String, Symbol> msorted = new TreeMap<>(); 
        TreeMap<String, Symbol> vsorted = new TreeMap<>(); 
  
        // Copy all data from hashMap into TreeMap 
        csorted.putAll(ctable); 
        msorted.putAll(mtable); 
        vsorted.putAll(vtable); 
  
        // Display the TreeMap which is naturally sorted 
        for ( Map.Entry<String,Symbol> entry : vsorted.entrySet() ) 
        {            
        ...
        for ( Map.Entry<String,Symbol> entry : csorted.entrySet() ) 
        {            
            String s = entry.getKey();
            Symbol sym = entry.getValue();         
            printTabs(level);
        ...
        for ( Map.Entry<String,Symbol> entry : msorted.entrySet() ) 
        {            
            String s = entry.getKey();
            Symbol sym = entry.getValue();         
            printTabs(level);
    ...
    public void printAll() {
    	for(Map.Entry<String, Symbol> entry : ctable.entrySet()) {
    	...
    	for(Map.Entry<String, Symbol> entry : mtable.entrySet()) {
    		String key = entry.getKey();
    		Symbol val = entry.getValue();
    		System.out.println(val.toString());
    	}
